from flask import Blueprint, request, jsonify
from flask_jwt_extended import jwt_required, get_jwt_identity

# Services
from documents.doc_services import upload_document, process_document

# Database
from database.extensions import db
from database.models import Document, Summarization  # <-- Added Summarization

doc_bp = Blueprint("document_bp", __name__)

# ---------------------------------------------------------
# 1. UPLOAD FILE
# ---------------------------------------------------------
@doc_bp.route("/upload", methods=["POST"])
@jwt_required()
def upload_file():
    user_id = get_jwt_identity()

    if not user_id:
        return jsonify({"status": "error", "message": "Invalid token or user not found"}), 401

    response, status = upload_document(user_id)
    return jsonify(response), status


# ---------------------------------------------------------
# 2. PROCESS FILE (Trigger NLP)
# ---------------------------------------------------------
@doc_bp.route("/process/<uuid:doc_id>", methods=["POST"])
@jwt_required()
def process_file(doc_id):
    """
    Triggers the expensive NLP pipeline. 
    Returns the result immediately upon completion.
    """
    response, status = process_document(doc_id)
    return jsonify(response), status


# ---------------------------------------------------------
# 3. GET ANALYSIS RESULTS (New Route)
# ---------------------------------------------------------
@doc_bp.route("/result/<uuid:doc_id>", methods=["GET"])
@jwt_required()
def get_analysis_results(doc_id):
    """
    Fetches the already generated Summary, Clauses, and Review 
    from the database without re-processing.
    """
    # 1. Check if summary exists for this doc
    summary = Summarization.query.filter_by(doc_id=doc_id).first()

    if not summary:
        return jsonify({
            "status": "error", 
            "message": "Analysis not found. Please click 'Process' first."
        }), 404

    # 2. Return the stored data
    return jsonify({
        "status": "success",
        "data": summary.to_dict()
    }), 200


# ---------------------------------------------------------
# 4. GET RAW TEXT (For debugging/viewing)
# ---------------------------------------------------------
@doc_bp.route("/extract/<uuid:doc_id>", methods=["GET"])
@jwt_required()
def extract_raw_text(doc_id):
    document = Document.query.get(doc_id)

    if not document:
        return jsonify({"status": "error", "message": "Document not found"}), 404

    return jsonify({
        "status": "success",
        "data": {
            "doc_id": str(document.doc_id),
            "status": document.status,
            "raw_text": document.raw_text
        }
    }), 200


# ---------------------------------------------------------
# 5. DELETE DOCUMENT
# ---------------------------------------------------------
@doc_bp.route("/delete/<uuid:doc_id>", methods=["DELETE"])
@jwt_required()
def delete_document(doc_id):
    document = Document.query.get(doc_id)
    if not document:
        return jsonify({"status": "error", "message": "Document not found"}), 404

    try:
        db.session.delete(document)
        db.session.commit()
        return jsonify({"status": "success", "message": "Document deleted"}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": str(e)}), 500